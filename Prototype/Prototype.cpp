...
struct WaveProcessingInfo
{
    ...
    std::shared_ptr<BinData::RawFile> inputFile;
    std::shared_ptr<BinData::RawFile> outputFile;
...
void PrintDescriptor(ChunkDescriptor& descriptor)
{
    ...
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Chunk ID     : " << descriptor.id << std::endl;
    std::cout << "Chunk Size   : " << descriptor.size << std::endl;
    std::cout << "Format       : " << descriptor.format << std::endl;
    std::cout << std::endl;
}
...
void PrintFormatInfo(FormatInfo& info)
{
    ...
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Audio Format : " << info.audioFormat << std::endl;
    std::cout << "Channels     : " << info.channels << std::endl;
    std::cout << "Sample Rate  : " << info.sampleRate << std::endl;
    std::cout << "Byte Rate    : " << info.byteRate << std::endl;
    std::cout << "Block Align  : " << info.blockAlign << std::endl;
    std::cout << "Bits / Sample: " << info.bitsPerSample << std::endl;
}
...
void UpdateFormatInfo(WaveProcessingInfo& info)
{
    ...
    info.newFormat.audioFormat = info.originalFormat.audioFormat;
    info.newFormat.channels = info.originalFormat.channels;
    info.newFormat.sampleRate = info.originalFormat.sampleRate;
    info.newFormat.blockAlign = info.originalFormat.blockAlign;
    info.newFormat.byteRate = info.originalFormat.byteRate;
    info.newFormat.bitsPerSample = info.originalFormat.bitsPerSample;
    ...
    info.newFormat.blockAlign = bytesPerSample * info.originalFormat.channels.Value();

    // Byte rate is the number of bytes per second give the sample rate.
    info.newFormat.byteRate = info.newFormat.blockAlign.Value() 
        * info.newFormat.sampleRate.Value();

    info.newFormat.bitsPerSample = bitsPerSample;
}
...
void Convert16To24Bit(WaveProcessingInfo& info)
{
    ...
    info.newDataHeader = info.originalDataHeader;
    info.newDataHeader.size = newDataSize;

    // The new RIFF chunk descriptor will stay the same as the origial except
    // for the size increase.
    info.newDescriptor.id = info.originalDescriptor.id;
    info.newDescriptor.size = info.originalDescriptor.size.Value() + sizeIncrease;
    info.newDescriptor.format = info.originalDescriptor.format;
    ...
    while (bytesRemaining > 0)
    {
        ...
        // afforded by 24-bits.
        BinData::Int24Field upscaledSample{ 0 };
...
{
    info.originalDataHeader = header;
    std::cout << std::endl;
...
void ProcessOtherSubchunk(SubChunkHeader& header, WaveProcessingInfo& info)
{
    ...
    auto subChunkData = std::make_shared<BinData::RawField>(header.size.Value());

    subChunkID->SetData(header.id.ToString());
...
int main(int argc, char** argv)
{
    ...
    info.inputFile = std::make_shared<BinData::RawFile>(fileReadStream);
    info.outputFile = std::make_shared<BinData::RawFile>(fileWriteStream);
